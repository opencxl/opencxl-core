    ...
    async def _process_upstream_host_to_target_packets(self):
        while True:
            ...
            if cache_packet.type == CACHE_REQUEST_TYPE.SNP_DATA:
                mem_packet = MemoryRequest(MEMORY_REQUEST_TYPE.READ, addr, cache_packet.size)
                ...
                cxl_packet = CxlCacheCacheH2DReqPacket.create(addr, CXL_CACHE_H2DREQ_OPCODE.SNP_INV)
                await self._downstream_cxl_cache_fifos.host_to_target.put(cxl_packet)

            elif cache_packet.type == CACHE_REQUEST_TYPE.SNP_CUR:
                cxl_packet = CxlCacheCacheH2DReqPacket.create(addr, CXL_CACHE_H2DREQ_OPCODE.SNP_CUR)
                await self._downstream_cxl_cache_fifos.host_to_target.put(cxl_packet)
    ...
    async def _process_downstream_target_to_host_packets(self):
        while True:
            ...
            if base_packet.is_d2hreq():
                ...
                elif packet.d2hreq_header.cache_opcode == CXL_CACHE_D2HREQ_OPCODE.CACHE_CL_FLUSH:
                    self._addr = addr
                    cxl_packet = CxlCacheCacheH2DRspPacket.create(
                ...
                elif packet.d2hreq_header.cache_opcode == CXL_CACHE_D2HREQ_OPCODE.CACHE_RD_ANY:
                ...
                mem_packet = MemoryRequest(MEMORY_REQUEST_TYPE.WRITE, self._addr, 64, packet.data)
                await self._memory_producer_fifos.request.put(mem_packet)
                ...
                if packet.d2hrsp_header.cache_opcode < CXL_CACHE_D2HRSP_OPCODE.RSP_S_FWD_M:
                    ...
                    elif packet.d2hrsp_header.cache_opcode == CXL_CACHE_D2HRSP_OPCODE.RSP_V_HIT_V:
                    ...
                    if packet.d2hrsp_header.cache_opcode == CXL_CACHE_D2HRSP_OPCODE.RSP_S_FWD_M:
                        ...
                        status = CACHE_RESPONSE_STATUS.RSP_I
                    elif packet.d2hrsp_header.cache_opcode == CXL_CACHE_D2HRSP_OPCODE.RSP_V_FWD_V:
                        status = CACHE_RESPONSE_STATUS.RSP_V
                    assert not self._downstream_cxl_cache_fifos.target_to_host.empty()
